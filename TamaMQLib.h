...
void GetPointByVidx(MQObject o, std::vector<int> &vidx, std::vector<MQPoint> &pts)
{
  int num = vidx.size();
  for(int i=0;i<num;i++)
  {
    int vi = vidx[i];
    if(o->GetVertexRefCount(vi)==0)  //ERROR
    {
      pts.clear();
      return;
    }
    pts.push_back(o->GetVertex(vi));
  }
}
MQPoint GetPointByVi(MQObject o, int vi)
{
  if(o->GetVertexRefCount(vi)==0)return MQPoint(0.0f, 0.0f, 0.0f);
  return o->GetVertex(vi);
}
...
int GetVertexRelatedFaces(MQObject o, int vi, std::vector<int> &fidx)
{
  int numRelF = o->GetVertexRelatedFaces(vi, NULL);
  fidx.resize(numRelF);
  o->GetVertexRelatedFaces(vi, &(fidx[0]));
  return numRelF;
}

void GetVertexColors(MQObject o, int fi, int numFV, std::vector<DWORD> &colors)
{
  for(int i=0;i<numFV;i++)
  {
    colors[i] = o->GetFaceVertexColor(fi, i);
  }
}
void SetVertexColors(MQObject o, int fi, int numFV, std::vector<DWORD> &colors)
{
  for(int i=0;i<numFV;i++)
  {
    o->SetFaceVertexColor(fi, i, colors[i]);
  }
}
void SetVertexColors2(MQObject o, int fi, std::vector<DWORD> &colors, int start, int num)
{
  int idxInMQFace = 0;
  for(int i=start; idxInMQFace<num; i++,idxInMQFace++)
  {
    o->SetFaceVertexColor(fi, idxInMQFace, colors[i]);
  }
}
